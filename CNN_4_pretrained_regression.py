...
#data =pd.read_csv('LablesForMeter.csv')
...
def nameToImage(x_name,image_size=224):
    #m=x_name.shape[0]
    x_image=np.zeros((3,image_size,image_size)).astype(np.uint8)
    
    path=root+str(x_name)
    I = cv2.imread(path)
    pic = cv2.resize(I, (image_size, image_size), interpolation=cv2.INTER_CUBIC)
    x_image[:,:,:]=moveaxis(pic,2, 0)
...
class Dataset(Data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        X = torch.FloatTensor(normalizedImage(nameToImage(ID)))
        
        #X=ID
        y1 = self.labels['labels'][ID]
        y2=self.labels['area'][ID]
...
flat_list = []
for sublist in x_train.values.tolist():
    for item in sublist:
            ...
            for inputs, labels, area in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if is_inception and phase == 'train':
                        ...
                        acc = torch.sum(1-0.5*criterion_2(outputs, label_and_area)/label_and_area)
